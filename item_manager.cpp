	...
	switch (id) {
		case GCCraftResponse_t::k_iMessage:
		{
			...
			if (response->blueprint == 0xffff) {
				...
				if (!view_->create_alert( &CRAFT_FAIL_MESSAGE )) {
				...
				case 7:
					{
						if (steam_items_.get_steam_id() == cache_msg.owner()) {
							for (int i = 0; i < subscribed_type.object_data_size(); i++) {
								...
								if (slots < inventory_inventory_book_.get_slot_count()) {
                                    stack->log( "New backpack size smaller than old." );
                                    return false;
								}

								// Add the newer slots.
								unsigned int added_slots = slots - inventory_inventory_book_.get_slot_count();
								unsigned int added_pages = added_slots / inventory_inventory_book_.get_page_size();
								inventory_inventory_book_.add_pages( added_pages );
							}
						}
