...
const Colour SLOT_STROKE_WHITE_COLOUR			= { 255, 255, 255 };
const Colour SLOT_BLACK_COLOUR					= { 0, 0, 0 };
...
void SlotView::Draw( Graphics2D* graphics )
{
	...
	if (slot_->HasItem()) {
		Item* item = slot_->GetItem();
		graphics->draw_texture( item->GetTexture(), 0.0f, 0.0f );
		if (item->IsEquipped()) {
...
void SlotView::Precache( Graphics2D* graphics )
{
	...
	stroke_			= new RoundedRectangle( SLOT_WIDTH, SLOT_HEIGHT, SLOT_RADIUS, SLOT_BLACK_COLOUR );
